...
$sql = "SELECT lid,title,price,spec,sold_count,is_onsale,fname FROM xz_laptop l LEFT JOIN xz_laptop_family f ON f.fid=l.family_id " . ($kw?" WHERE title LIKE '%$kw%'":"") . " ORDER BY sold_count DESC LIMIT $start,$count";
 $result = mysqli_query($conn,$sql);
